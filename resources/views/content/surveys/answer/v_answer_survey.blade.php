        ...
        @foreach ($surveys as $survey)
                ...
                <div class="row justify-content-around align-items-center" style="margin-bottom: 25px;">
                    ...
                    <div style="width: 50%" class="d-flex justify-content-center justify-content-sm-around">
                        @foreach ($options as $key => $option)
                            <label> <input type="radio" value="{{ $key }}" {{ $key == 3 ? 'checked' : '' }}
                                    class="option-input radio r" name="val-{{ $no }}"></label>
                        @endforeach
                    </div>
                    <input type="hidden" name="type-{{ $no }}" value="{{ $survey['type'] }}">
                    <input type="hidden" name="key-{{ $no }}" value="{{ $survey['id'] }}">
            ...
            @else
                <div class="form-control">
                    ...
                    <textarea name="val-{{ $no }}" placeholder="Enter your comment here" required></textarea>
                    <input type="hidden" name="type-{{ $no }}" value="{{ $survey['type'] }}">
                    <input type="hidden" name="key-{{ $no }}" value="{{ $survey['id'] }}">
                </div>
                <hr>
